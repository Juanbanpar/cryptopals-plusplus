...
static double normalized_hamming_for_keysize(const std::vector<unsigned char> &data, int keysize) {
  ...
  if (blocks < 2) return std::numeric_limits<double>::infinity();
  double total = 0.0;
  int pairs = 0;
...
std::pair<std::string, std::string> set01::ex06::break_repeating_key_xor_from_base64(const std::string &b64) {
  ...
  for (int t = 0; t < try_count; ++t) {
    ...
    for (size_t i = 0; i < data.size(); ++i) blocks[i % keysize].push_back(data[i]);
    ...
    for (size_t i = 0; i < data.size(); ++i) plain[i] = static_cast<char>(data[i] ^ (unsigned char)key[i % keysize]);
