...
#define Nr 10
    ...
    std::vector<uint8_t> key_expansion(const std::vector<uint8_t> &key)
    {
        ...
        while (i < Nk)
            i++; // First 4 words are the key itself, already copied
    ...
    std::vector<uint8_t> encrypt_ecb(const std::vector<uint8_t> &plaintext, const std::vector<uint8_t> &key, bool pad)
    {
        ...
        if (pad)
            pkcs7_pad_vec(inp, 16);

        if (inp.size() % 16 != 0)
            throw std::invalid_argument("Plaintext size not multiple of 16 and padding disabled");
    ...
    std::vector<uint8_t> decrypt_ecb(const std::vector<uint8_t> &ciphertext, const std::vector<uint8_t> &key, bool unpad)
    {
        if (ciphertext.size() % 16 != 0)
            throw std::invalid_argument("Ciphertext size not multiple of 16");
    ...
    std::vector<uint8_t> ctr(const std::vector<uint8_t> &data, const std::vector<uint8_t> &key, uint64_t nonce)
    {
        ...
        for (size_t i = 0; i < data.size(); i += 16)
        {
            ...
            uint8_t counter[16];
            for (int j = 0; j < 8; ++j)
                counter[j] = (nonce >> (j * 8)) & 0xFF;
            for (int j = 0; j < 8; ++j)
                counter[8 + j] = (block_count >> (j * 8)) & 0xFF;
            ...
            const size_t bytes_to_xor = std::min<size_t>(16, data.size() - i);
