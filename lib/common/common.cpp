...
std::vector<unsigned char> rand_bytes(size_t n)
{
    ...
    for (size_t i = 0; i < n; ++i)
        out[i] = static_cast<unsigned char>(dis(gen));
    return out;
}
...
std::string fixed_xor_hex(const std::string &ahex, const std::string &bhex)
{
    ...
    for (size_t i = 0; i < a.size(); ++i)
        out.push_back(static_cast<unsigned char>(a[i] ^ b[i]));
...
double score_english(const std::string &s)
{
    ...
    if (!init)
    {
        ...
        for (size_t i = 0; i < letters.size(); ++i)
            table[(unsigned char)letters[i]] = (double)(letters.size() - i);
    }
    double score = 0.0;
...
std::pair<unsigned char, std::string> crack_single_byte_xor_hex(const std::string &hex)
{
    ...
    for (int k = 0; k < 256; ++k)
    {
        ...
        for (size_t i = 0; i < bytes.size(); ++i)
            out[i] = static_cast<char>(bytes[i] ^ (unsigned char)k);
...
{
    if (a.size() != b.size())
        throw std::invalid_argument("hamming: inputs must have equal length");
    int dist = 0;
    for (size_t i = 0; i < a.size(); ++i)
    {
